...
import cimento_50kg.service.EnderecoFornecedorService;
...
@Path("/enderecos-fornecedor")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class EnderecoFornecedorResource {

    @Inject
    EnderecoFornecedorService service;

    @GET
    public List<EnderecoFornecedorResponseDTO> listar() {
    ...
    @POST
    public Response criar(EnderecoFornecedorRequestDTO dto) {
        return Response.status(Response.Status.CREATED).entity(service.criar(dto)).build();
    }

    @PUT
    @Path("/{id}")
    public Response atualizar(@PathParam("id") Long id, EnderecoFornecedorRequestDTO dto) {
